function [R,t,X,Q,a,pk,T] = jrmpc_Nooutliers(V,X,varargin)
...
for iter = 1:maxNumIter
 ...
 %  a(find(all(cellfun(@(x) any(isnan(x)),a))==1))={0};
    % ------  weighted UMEYAMA ------ 
  ...
  lambda = cellfun(@(a) sum(a)',a,'uniformoutput',false); % 1 x K rows
    ...
    %%%%%%%%%%%%
  %    R2 = cellfun(@(uu,vv) uu*diag([1 1 det(uu*vv)])*vv',vv,uu,'uniformoutput',false);
    % solve for t
